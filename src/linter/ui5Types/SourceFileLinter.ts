	...
	visitNode(node: ts.Node) {
		if (node.kind === ts.SyntaxKind.NewExpression) { // e.g. "new Button({\n\t\t\t\tblocked: true\n\t\t\t})"
			...
			if (nodeIsUiComponent) {
				...
				nodeIsControl = this.isUi5ClassDeclaration(node, "sap/ui/core/Control");
				if (nodeIsControl) {
					nodeIsManagedObject = true;
				} else {
			...
			if (nodeIsControl) {
				this.analyzeControlRendererDeclaration(node as ts.ClassDeclaration);
				this.analyzeControlRerenderMethod(node as ts.ClassDeclaration);
			}
		}
