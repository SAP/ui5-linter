	...
	_handleUi5LibraryNamespace(
		...
		if (/^[a-z]/.exec(moduleName)) {
			...
			for (const prop of controlProperties) {
				// TODO: Check whether prop is of type "property" (indicating that it can have a binding)
				// Depends on https://github.com/SAP/ui5-linter/pull/452
				this.#bindingLinter.lintBinding(prop.value, this.#requireDeclarations, prop.start);
			}
